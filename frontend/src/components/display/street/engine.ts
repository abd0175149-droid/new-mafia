...
/** الأصول المخفَّفة (gltf-transform: weld → simplify → join → flatten): 1.37M مثلّث في المشهد كانت أغلبها براغي أعمدة الكهرباء والمصابيح والحنفيّة */
const LITE_PH = new Set(['modular_electricity_poles', 'street_lamp_01', 'street_lamp_02', 'fire_hydrant', 'wooden_barrels_01', 'metal_trash_can', 'cardboard_box_01', 'wooden_crate_01', 'wooden_crate_02', 'outdoor_table_chair_set_01', 'planter_box_01', 'trashbag', 'wooden_ladder', 'water_manhole_cover', 'standing_chalkboard_01', 'painted_wooden_bench', 'modular_fire_escape', 'covered_car']);
const LITE_SF = new Set(['pierce_arrow', 'coupe33', 'awning', 'clothesline', 'diorama1930', 'brownstone', 'fedoras']);
const PH = (n: string) => LITE_PH.has(n) ? `${ASSET_ROOT}/models/${n}/${n}.lite.glb` : `${ASSET_ROOT}/models/${n}/${n}.gltf`;
const SF = (n: string) => LITE_SF.has(n) ? `${ASSET_ROOT}/sketchfab/${n}/scene.lite.glb` : `${ASSET_ROOT}/sketchfab/${n}/scene.gltf`;
...
class StreetEngine {
  ...
  lamps: Lamp[] = []; neons: Neon[] = []; clubLights: THREE.PointLight[] = []; laundry: { m: THREE.Object3D; ph: number }[] = []; pigeons: { s: THREE.Sprite; t: number; ox: number; oz: number }[] = [];
  ...
  STEAM_O = new THREE.Vector3(2.2, .1, -14); EXH_O = new THREE.Vector3(4.7, .4, -6.6);
  /** للشارة التشخيصيّة */
  ...
  private build() {
    ...
    ([-1, 1] as const).forEach(s => { const sw = new THREE.Mesh(new THREE.BoxGeometry(SIDE_W, .16, 220), MAT.slab); sw.position.set(s * (STREET_W / 2 + SIDE_W / 2), .08, -60); sw.receiveShadow = true; S.add(sw); /* 🔴 وجهُ الحافّة نحو الشارع كان منطبقاً على وجه صندوق الرصيف الجانبيّ (x=±6 كلاهما) فيتنازعان العمق (z-fighting) ويظهر شريطٌ فاتح متقطّع بلون البلاط — الحافّةُ تبرز 6 سم أمام الرصيف */ const curb = new THREE.Mesh(new THREE.BoxGeometry(.24, .18, 220), new THREE.MeshStandardMaterial({ color: 0x4a4946, roughness: 1, metalness: 0, envMapIntensity: .12 })); /* غرانيتٌ خشن — كان يلمع بيجاً بانعكاس بيئة الفجر */ curb.position.set(s * (STREET_W / 2 + .06), .09, -60); S.add(curb);
    ...
    const LAMPS: [number, number][] = [[-6.2, -2], [6.2, -12], [-6.2, -24], [6.2, -36], [-6.2, -48], [6.2, -60]];
    ...
    /* عند الرصيف (حافّته ±6): كانتا على ±3.5 أي في حارة السير على 1.6م من الرصيف */ this.car.position.set(4.9, 0, -9); this.car.rotation.y = Math.PI / 2 + .05; S.add(this.car); this.car2.position.set(-4.9, 0, -31); this.car2.rotation.y = -Math.PI / 2 - .04; S.add(this.car2);
    ...
    const STEAM_O = new THREE.Vector3(2.2, .1, -14); this.steam = this.particles(14, 0x8a8580, .7, STEAM_O, 1.4); this.purple = this.particles(44, 0x5a2ea0, 1.5, STEAM_O, 2.4); this.exhaust = this.particles(6, 0x777780, .4, new THREE.Vector3(4.7, .4, -6.6), .4);
  ...
  pinRigidProps(root: THREE.Object3D) {
    ...
    const c = new THREE.Vector3(), bp = new THREE.Vector3(); let n = 0;
   ...
   * بمواضعها العالميّة (واجهات، مصابيح، أعمدة، صناديق…) — من ~750 نداء رسم إلى بضع عشرات، بلا تغيير في الشكل.
   * الشخصيّات والسيّارات والأجسام المتحرّكة (القبّعة، الرذاذ) لا تُدمج.
   */
  mergeStatic() {
    if (this.disposed) return; const S = this.scene; S.updateMatrixWorld(true);
    const groups = new Map<string, { mat: THREE.Material; geos: THREE.BufferGeometry[]; cast: boolean }>(); const remove: THREE.Mesh[] = [];
    S.traverse(o => { const m = o as THREE.Mesh; if (!m.isMesh || !m.userData.mergeStatic || (m as unknown as THREE.SkinnedMesh).isSkinnedMesh || (m as unknown as THREE.InstancedMesh).isInstancedMesh || Array.isArray(m.material)) return; if (!m.visible) return;
      const g = m.geometry; if (!g.attributes.position) return; const key = (m.material as THREE.Material).uuid + '|' + Object.keys(g.attributes).sort().join(',') + '|' + (g.index ? 'i' : 'n');
      const gg = g.clone(); deinterleaveGeometry(gg); /* ملفّات gltf-transform تأتي بسماتٍ متداخلة والدمج لا يقبلها */ gg.applyMatrix4(m.matrixWorld); if (gg.attributes.normal) gg.normalizeNormals(); const grp = groups.get(key) || { mat: m.material as THREE.Material, geos: [], cast: false }; grp.geos.push(gg); grp.cast = grp.cast || m.castShadow; groups.set(key, grp); remove.push(m); });
  ...
  private buildBrownstoneStreet(src: THREE.Object3D) {
    // 🏛️ قطعٌ مختارة (تدقيق 2026-09-25): مكيّفاتُ النوافذ مطبوعةٌ في قوام أكثر قطع الطقم — لا تُخفى
    //    (شبكات Maskedstuff هي سلالمُ الحريق لا المكيّفات). فتُحصر الواجهاتُ في الخالية منها:
  ...
  blobShadow(size = 1.2): THREE.Mesh {
    if (!this.blobTex) { const c = document.createElement('canvas'); c.width = c.height = 64; const x = c.getContext('2d')!; const g = x.createRadialGradient(32, 32, 4, 32, 32, 30); g.addColorStop(0, 'rgba(0,0,0,.55)'); g.addColorStop(.6, 'rgba(0,0,0,.22)'); g.addColorStop(1, 'rgba(0,0,0,0)'); x.fillStyle = g; x.fillRect(0, 0, 64, 64); this.blobTex = new THREE.CanvasTexture(c); }
    const m = new THREE.Mesh(new THREE.PlaneGeometry(size, size * .7), new THREE.MeshBasicMaterial({ map: this.blobTex, transparent: true, depthWrite: false, toneMapped: false })); m.rotation.x = -Math.PI / 2; m.position.y = .005; m.renderOrder = 1; return m;
  }
  /* ── crowd & animation ── */
  ...
  /**
   * إعادة توجيه حركةٍ من هيكل Mixamo إلى هيكل الشخصيّة **بمحاذاة اتّجاه العظام**:
   *   Wt(t) = Ws(t) · Ws0⁻¹ · A_b · Wt0
   * حيث A_b أقصرُ دورانٍ يحمل اتّجاهَ العظمة في راحة الهدف إلى اتّجاهها في راحة المصدر.
   * أي: يُوضع الهدف أوّلاً في وضعيّة راحة المصدر (T) بأقلّ التواءٍ ممكن، ثمّ تُساق عظامه
   * بدورانات المصدر العالميّة — فيكون اتّجاهُ كلّ عظمةٍ في العالم مطابقاً للمصدر إطاراً إطاراً.
   * ثمّ يُحوَّل الناتج إلى دورانٍ محلّيّ هرميّاً.
   *
   * 🔴 تاريخُ الخطأ (2026-09-25) — صيغتان سابقتان بدتا صحيحتين لأنّ مقياسي كان خاطئاً:
   *    • دلتا عالميّة Ws(t)·Ws0⁻¹·Wt0: تحتسب فرقَ راحة T→A مرّتين ⇒ اليدان خلف الظهر.
   *    • إطارُ الراحة Wt0·Ws0⁻¹·Ws(t): تفترض تطابقَ محاور العظام المحلّيّة، وهي لا تتطابق حتّى
   *      بين ملفَّي Mixamo مرّا بمسارَي تحويلٍ مختلفَين ⇒ الذراع تلتفّ إلى داخل الصدر.
   *    كنتُ أقيس «مسافة اليد عن الورك» وهي عمياء عن يدٍ تخترق الجسد. المقياسُ الصحيح هو
   *    **زاوية اتّجاه كلّ عظمة مقارنةً بالمصدر** (`scripts/check-retarget.mjs`):
   *      mafia_boss:    دلتا عالميّة 47° · إطار الراحة 46° · محاذاة 0°   (العضد، متوسّط الدورة)
   *      gangster_lite: دلتا عالميّة 47° · إطار الراحة 50° · محاذاة 0°
   *    فالصيغةُ واحدةٌ لكلّ الهياكل، ولا حاجة للتفريق بينها.
   *
   *    A_b يُحسب من العظمة إلى ابنها «الرئيس» (العمود الفقريّ يُفضَّل على الساقين عند الورك،
   *    والرقبة على الكتفين عند الصدر)؛ والطرفُ الذي لا ابنَ له يرث محاذاة أبيه.
  ...
  async retargetLocal(target: THREE.SkinnedMesh, srcHips: THREE.Bone, clip: THREE.AnimationClip, names: Record<string, string>, grounded = false, label = '', planar = false): Promise<THREE.AnimationClip> {
    ...
    // A_b: محاذاةُ اتّجاه كلّ عظمة (الهدف → المصدر) في وضعيّة الراحة
    const tPos = new Map<THREE.Bone, THREE.Vector3>(); ordered.forEach(b => tPos.set(b, b.getWorldPosition(new THREE.Vector3())));
    const sPos = new Map<string, THREE.Vector3>(); srcByName.forEach((b, k) => sPos.set(k, b.getWorldPosition(new THREE.Vector3())));
    const isTrunk = (b: THREE.Object3D) => /Spine|Neck|Head/.test(coreBoneName(b.name) || '');
    const align = new Map<THREE.Bone, THREE.Quaternion>();
    ordered.forEach(b => {
      const sn = names[b.name]; if (!sn || !srcByName.has(sn)) return;
      const kids = b.children.filter(c => (c as THREE.Bone).isBone && names[c.name] && srcByName.has(names[c.name])) as THREE.Bone[];
      const child = kids.find(isTrunk) || kids[0];
      let A: THREE.Quaternion | null = null;
      if (child) {
        const dT = tPos.get(child)!.clone().sub(tPos.get(b)!), dS = sPos.get(names[child.name])!.clone().sub(sPos.get(sn)!);
        if (dT.lengthSq() > 1e-10 && dS.lengthSq() > 1e-10) A = new THREE.Quaternion().setFromUnitVectors(dT.normalize(), dS.normalize());
      }
      if (!A) { const p = b.parent as THREE.Bone | null; A = (p && align.get(p)) ? align.get(p)!.clone() : new THREE.Quaternion(); }
      align.set(b, A);
    });
    const offset = new Map<THREE.Bone, THREE.Quaternion>(); ordered.forEach(b => { const sn = names[b.name]; if (!sn || !srcByName.has(sn)) return; offset.set(b, srcRestWorld.get(sn)!.clone().invert().multiply(align.get(b)!).multiply(restWorld.get(b)!)); });
  ...
  setMode(m: StreetMode, opts?: { instant?: boolean }) {
    ...
    this.applyCrowdMode(); this.scene.environment = m === 'night' ? this.envNight : this.envDawn; this.applyQuality(); this.renderer.shadowMap.needsUpdate = true;
  }
  ...
  /** offline capture (app welcome video): fixed size, fixed-step frames, each frame returned as a data URL via onFrame */
  async captureSequence(shot: string, seconds: number, fps: number, w: number, h: number, onFrame: (dataUrl: string, i: number) => Promise<void> | void, mode: StreetMode = 'night') {
    await this.assetsReady; if (!this.prewarmDone) await this.prewarm(); this.active = false; this.mode = mode; this.applyPreset(mode === 'night' ? this.NIGHT : mode === 'dawn' ? this.DAWN : this.DAY); this.applyCrowdMode(); this.scene.environment = mode === 'night' ? this.envNight : this.envDawn;
    this.renderer.setPixelRatio(1); this.renderer.setSize(w, h, false); this.camera.aspect = w / h; this.camera.clearViewOffset(); this.camera.updateProjectionMatrix(); this.composer.setSize(w, h); this.bloom.setSize(w, h); this.smaa.setSize(w, h);
    const sh = this.shots[shot]; if (!sh) return; const n = Math.round(seconds * fps); this._p.copy(sh.at(0)[0]); this._l.copy(sh.at(0)[1]);
  ...
  private loop = () => {
    ...
    if (this.shadowEvery && this.frameIndex % this.shadowEvery === 0) this.renderer.shadowMap.needsUpdate = true;
