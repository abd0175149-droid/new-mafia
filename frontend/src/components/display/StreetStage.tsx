...
export default function StreetStage({ mode, event, eventKey, docked, ambient, debug, hint }: { mode: StageMode; event?: string | null; eventKey?: string | number; docked?: boolean; ambient?: boolean; debug?: boolean; hint?: string | null }) {
  ...
  useEffect(() => {
    ...
    if (hide) return; const t = setInterval(() => { const e = getStreetEngine(); if (!e) { setStats('NO WEBGL'); return; } const st = e.stats(); const exr = e.exec ? (e.exec.ready() ? (st.quality === 'low' ? 'EXEC OFF(low)' : `EXEC READY(${e.exec.tpl.size})`) : 'EXEC OFF(no-crowd)') : 'EXEC ?'; setStats(`${exr} · GPU ${String(st.gpu).replace(/ANGLE \((.*?)\)/, '$1').replace(/Direct3D.*$/, '').replace(/,\s*$/, '').slice(0, 42)} · ENV ${st.mode.toUpperCase()} · QUALITY ${st.quality.toUpperCase()}${st.ambient ? ' (AMBIENT)' : ''} · ${tier === 'poster' ? 'STATIC POSTER' : 'LIVE 3D'} · ${st.ready ? (st.prewarm ? `WARM ${st.prewarmMs}ms` : 'WARMING') : 'LOADING'} · ${st.fps} FPS · ${st.tris} TRIS · ${st.calls} CALLS · LAST SWITCH ${st.lastTransitionMs}ms`); }, 1000); return () => clearInterval(t);
  }, [debug, tier, mode]);
