...
// ── 🪑 ترحيل المفكرة بعد إعادة ترتيب المقاعد ──
// المفكرة هي الشيء الوحيد الذي يملكه الجهاز ولا يستطيع الخادم إعادة إرساله،
// فهي تُرحَّل لا تُمحى: الدلو ينتقل لمفتاح مقعدي الجديد، ومفاتيحه الداخلية
// (مقاعد الآخرين) تمرّ بخريطة الخادم. المفتاح 0 = الملاحظة العامّة بلا مقعد.
// ممنوع أيّ حسابٍ محلّي للأرقام — الخريطة وحدها هي المرجع.
...
export default function PlayerFlow({ initialRoomCode = '', inviteFlag = false, inviterName = '' }: PlayerFlowProps) {
  ...
  // 🪑 عدّاد إعادة ترتيب المقاعد — يُجبر المفكرة على إعادة قراءة دلوها بعد الترحيل
  const [notepadRemapNonce, setNotepadRemapNonce] = useState(0);
  // 🪑 نافذة كتم بانر «تم تغيير رقمك» المشتقّ من الاستطلاع/المزامنة: التوست الخاصّ
  // بإعادة الترتيب قالها بالفعل، فلا نُكرّرها ببانرٍ ثانٍ فوقه.
  const seatRemapUntilRef = useRef(0);
  // 🔄 آخر نسخة من دالة الاستطلاع — كي يستدعيها من يحتاج مزامنة فوريّة موثوقة
  // (إعادة ترتيب المقاعد) بدل إعادة بناء نفس النداء بحالةٍ ناقصة
  const pollStateRef = useRef<(() => void) | null>(null);
  // 🪑 آخر player:seat-changed وصل — مصدر «مقعدي قبل النقل» لمعالج إعادة الترتيب
  const lastSeatChangeRef = useRef<{ oldPhysicalId: number; newPhysicalId: number; at: number } | null>(null);
  // 🪑 نافذة «إعادة الدفع بعد النقل»: خلالها player:role-assigned إعادةُ تسليمٍ لا لعبةٌ
  // جديدة — فلا تُقلب البطاقة ولا يُطلق التنبيه ولا يُحيا لاعبٌ مُقصى.
  ...
  useEffect(() => {
    ...
    try {
      ...
      emit('room:rejoin-player', {
        ...
        // 🪑 الهوية أولاً: بعد نقل المقاعد لم يعُد المقعد المحفوظ دليلاً على صاحبه —
        // الحساب هو ما يعرّف اللاعب، والمقعد آخر ما يلجأ إليه الخادم
        ...
        if (res.success) {
          ...
          // 🪑 المقعد وحده لم يعُد يُثبت الهويّة (نقل/تبديل مقاعد) → نُعيده لشاشة الدخول
          // برسالة الخادم بدل تركه عالقاً على جلسةٍ لا يستطيع أحد التحقّق من صاحبها
          localStorage.removeItem('mafia_session');
          setApiError(res.error || 'تعذّر التعرّف عليك — أعد الدخول برقم هاتفك أو من حسابك');
  ...
  useEffect(() => {
    ...
    const cleanupReconnect = on('connect', () => {
      ...
      emit('room:rejoin-player', {
        ...
        playerId: playerId || getSavedPlayerId() || undefined, // 🪑 الهوية أولاً — المقعد قد يكون تغيّر
      }).then((res: any) => {
        if (res?.code === 'IDENTITY_REQUIRED') {
          // تعذّر التعرّف بعد انقطاع → لا نتركه على شاشة لعبةٍ لم يعُد الخادم يعترف بها
          localStorage.removeItem('mafia_session');
          setApiError(res.error || 'تعذّر التعرّف عليك — أعد الدخول برقم هاتفك أو من حسابك');
          setStep(initialRoomCode ? 'phone' : 'code');
          return;
  ...
  useEffect(() => {
    ...
    const cleanupSeat = on('player:seat-changed', (data: { oldPhysicalId: number; newPhysicalId: number }) => {
      // 🪑 يُبثّ قبل room:seats-remapped مباشرة → نحفظه ليعرف معالج الترتيب «مقعدي قبل النقل»
    ...
    // ══════════════════════════════════════════════════════
    // 🪑 إعادة ترتيب المقاعد — عقد التصالح: امحُ ← رحّل ← اسأل ← أعد الاشتقاق
    // ══════════════════════════════════════════════════════
    // ينفّذه **كل** من في الغرفة لا المنقولَين فقط: كل خريطة مفهرسة بالمقاعد
    // على أيّ جهاز صارت تشير للشخص الخطأ. وممنوع على الجهاز أن «يُصلح» ذاكرته
    // حسابياً — يمحوها ثم يسأل الخادم الذي يحلّ الهوية بالحساب/الهاتف لا بالمقعد.
    const cleanupRemap = on('room:seats-remapped', (data: { map: Record<string, number>; swapped: boolean; at: number }) => {
      const map = data?.map || {};

      // «مقعدي قبل النقل»: الحدث السابق قد يكون حدّث الرقم فعلاً، فنأخذه منه
      // متى كان طازجاً ومطابقاً للخريطة (التبديل لا يُميَّز بالعكس: 5→7 و7→5).
      ...
      // ①  امحُ كل ما هو مفهرس بالمقاعد ولا يمكن الوثوق به بعد الترقيم الجديد
      setVotingCandidates([]);
      ...
      setPhasePollData(null);        // تبرير/نقاش/إقصاء/اتفاقيات — كلها بأرقام مقاعد
      setRoster([]);                 // الروستر يعود كاملاً من أوّل استطلاع
      setVoiceMaps({ videoByPid: {}, audioByPid: {} }); // كاميرا/كلام على كارت الشخص الخطأ
      // نافذة الليل المفتوحة تُغلق: شبكة أمان الليل تعيد فتحها من الحالة الطازجة
      ...
      // العمدة: برومبت قراره وشارته وإعلانه — ثلاثتها مربوطة برقم مقعد
      setMayorPrompt(null);
      setMayorBanner(null);
      setMayorRevealedId(null);
      // 🛡️ نافذة صمت قصيرة لإعادة الدفع الخاصّة: الخادم يُعيد إرسال player:role-assigned
      //    لكل منقول (بلا حقل حياة)، ومعالجُه يفترض «دور جديد» فيُحيي ميتاً ويقلب بطاقته
      //    ويُطلق تنبيه الدور. الاستطلاع التالي هو المرجع لحالة الحياة.
      seatRemapRepushUntilRef.current = Date.now() + 5000;

      // ②  رحّل ما يملكه الجهاز ولا يُعيده الخادم: المفكرة الخاصّة + هويّات الفريق
      //     (الأسماء تسافر مع الأشخاص، فتمرّ أرقامها بخريطة الخادم كما فعل هو بحالته)
      ...
      // ③  اسأل الخادم (نفس نداء الاستطلاع) — هو المرجع لمقعدي ودوري ومرحلتي
      // (سجلّ دردشة المافيا أُعيد ترقيمه على الخادم أيضاً — تُعيد المفكرة جلبه بالعدّاد أعلاه)
      seatRemapUntilRef.current = Date.now() + 6000; // يكتم بانر «تم تغيير رقمك» المكرّر
      // ⏱️ تأجيل نداء الاستطلاع لدورة رسم واحدة: الاستدعاء الفوري ينفّذ الإغلاق (closure)
      //    المُلتقط **قبل** تحديث المقعد، فيستعيد صوتي من مفتاح المقعد القديم — أي صوت
      //    شريك التبديل. بعد الرسم يصير الإغلاق محدّثاً بالمقعد الجديد.
      setTimeout(() => pollStateRef.current?.(), 0);

      // ④  توست قصير غير مُقلق — بلا أسماء ولا تفاصيل تكشف من تحرّك
      //     (بلا اهتزاز: من انتقل مقعده اهتزّ جهازه أصلاً مع player:seat-changed)
      const msg = myNewSeat !== myOldSeat ? `تغيّر مقعدك إلى رقم ${myNewSeat}` : 'أُعيد ترتيب المقاعد';
  ...
  useEffect(() => {
    ...
    const cleanupRole = on('player:role-assigned', (data: { role: string; mafiaTeam?: {physicalId: number; name: string; role: string; avatarUrl?: string | null}[]; sibling?: any }) => {
      // 🪑 إعادة دفع بعد نقل مقعد (لا لعبة جديدة): الخادم يُعيد إرسال الدور ليصل لصاحبه
      //    بالمقعد الجديد. لا يجوز عندها قلب البطاقة ولا إطلاق تنبيه الدور ولا «إحياء» ميت.
    ...
    const cleanupSync = on('game:state-sync', (state: any) => {
      ...
      if (me) {
        ...
        if (String(me.physicalId) !== physicalId) {
          ...
          localStorage.setItem('mafia_session', JSON.stringify(saved));
          // تنبيه بصري — إلا إذا كان توست إعادة ترتيب المقاعد قد قالها للتو
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (res.success && res.player) {
          ...
          if (String(res.player.physicalId) !== physicalId) {
            ...
            saved.physicalId = res.player.physicalId;
            localStorage.setItem('mafia_session', JSON.stringify(saved));
            // تنبيه — إلا إذا كان توست إعادة ترتيب المقاعد قد قالها للتو
    ...
    // 🪑 ونُبقي مرجعاً لأحدث نسخة كي يستدعيها معالج إعادة ترتيب المقاعد فوراً
    ...
    if (emit && effectiveRoomId && playerPhone) {
      try {
        ...
        const res: any = await emit('room:rejoin-player', {
          ...
          phone: normalized,
          playerId: pid || undefined, // 🪑 الحساب أوثق من المقعد — الخادم يجرّبه أولاً
        });
        if (res?.success && res.player) {
          setPhysicalId(String(res.player.physicalId));
          setDisplayName(res.player.name);
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center font-sans relative overflow-hidden selection:bg-[#8A0303] selection:text-white ${isRemote ? 'justify-start p-2 pt-3 pb-24 bg-[#050505] remote-vignette' : 'justify-center p-4 sm:p-6 display-bg blood-vignette'}`}>
                  ...
                  onClick={() => {
                    ...
                    if (switchConfirm && emit) {
                      ...
                      emit('room:rejoin-player', {
                        ...
                        physicalId: 0,
                        phone: normalized,
                        playerId: playerId || getSavedPlayerId() || undefined, // 🪑 الهوية أولاً
                      }).then((res: any) => {
                        if (res?.success && res.player) {
                          ...
                          // 🪪 الخادم لم يتعرّف علينا (لا حساب ولا هاتف مطابق) — لا نتركه
                          // معلّقاً على زرٍّ لا يفعل شيئاً: نعيده لشاشة الدخول برسالته
                          setActiveToast({ message: res.error || 'تعذّر التعرّف عليك — أعد الدخول', type: 'warning' });
                          setStep('phone');
                        }
                      }).catch((e: any) => {
                        if (e?.response?.code === 'IDENTITY_REQUIRED') {
                          setActiveToast({ message: e.response.error || 'تعذّر التعرّف عليك — أعد الدخول', type: 'warning' });
                          setStep('phone');
                        }
                      });
                    }
      ...
      <PlayerNotepad
        ...
        /* 🪑 يتغيّر بعد إعادة ترتيب المقاعد: يُجبر المفكرة على إعادة قراءة دلوها
           المُرحَّل وإعادة جلب سجلّ التشاور (كلاهما أُعيد ترقيمه) */
