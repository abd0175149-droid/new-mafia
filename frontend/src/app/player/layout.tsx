...
function PlayerLayoutBody({ children }: { children: React.ReactNode }) {
  ...
  //
  // 📍 لكنّ بوّابةَ الموقع تُركَّب هنا أيضاً (قرار المالك): الإذن يُطلب فورَ فتح
  //    التطبيق لا بعد الدخول. وهي لا تطلب شيئاً بلا تمهيدٍ يشرح السبب — ورفضٌ
  //    واحدٌ على الويب لا يُعاد السؤال بعده برمجيّاً أبداً.
  //
  // 🔴 الإذنُ يُطلب هنا، أمّا **التبليغ** فيبقى بعد الدخول: player_last_fix
  //    يشترط player_id، فلا موقعَ يُخزَّن لمن لا هويّةَ له. البوّابة تُبلّغ
  //    لحظةَ ظهور الرمز.
  if (isPublic) {
    return <>{children}<LocationGate /></>;
  }
