...
// ── مكون قسم قابل للطي ──
function Section({ 
  id, title, icon, activeSection, setActiveSection, children 
}: { 
  id: string; title: string; icon: string; 
  activeSection: string | null; setActiveSection: (val: string | null) => void;
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
            ...
            <div className="p-4 space-y-4 border-t border-gray-700/20">{children}</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <Section id="basic" title="المعلومات الأساسية" icon="📝" activeSection={activeSection} setActiveSection={setActiveSection}>
          ...
          <div>
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="وصف اختياري للنشاط..." rows={3}
        ...
        <Section id="settings" title="الحالة والإعدادات" icon="⚙️" activeSection={activeSection} setActiveSection={setActiveSection}>
            ...
            <div className="grid grid-cols-2 gap-2">
              {STATUS_OPTIONS.map(opt => (
                <button key={opt.value} type="button" onClick={() => setStatus(opt.value)}
                  className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-sm font-medium transition-all ${status === opt.value ? opt.color + ' ring-1 ring-current' : 'bg-gray-900/40 border-gray-700/30 text-gray-500 hover:border-gray-600/50'}`}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" min="4" max="100" value={maxCapacity} onChange={e => setMaxCapacity(e.target.value)}
                className="w-full px-4 py-3 bg-gray-900/60 border border-gray-600/50 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-amber-500/50 text-sm" />
            </div>
            <div>
              ...
              <div className="flex gap-1.5">
                {DIFFICULTY_OPTIONS.map(opt => (
                  <button key={opt.value} type="button" onClick={() => setDifficulty(opt.value)}
                    className={`flex-1 flex flex-col items-center gap-0.5 py-2 rounded-lg border text-xs transition-all ${difficulty === opt.value ? 'bg-amber-500/15 border-amber-500/30 text-amber-400 ring-1 ring-amber-500/30' : 'bg-gray-900/40 border-gray-700/30 text-gray-500 hover:border-gray-600/50'}`}>
                    <span>{opt.icon}</span><span className="text-[10px]">{opt.label}</span>
                  </button>
                ))}
              </div>
            </div>
        ...
        <Section id="drive" title="Google Drive" icon="📂" activeSection={activeSection} setActiveSection={setActiveSection}>
          <div>
            ...
            <input type="text" value={driveLink} onChange={e => setDriveLink(e.target.value)} placeholder="https://drive.google.com/..." dir="ltr"
          ...
          {driveLink && (
            <a href={driveLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs text-blue-400 hover:text-blue-300 transition">
              📂 فتح المجلد في Drive ←
            </a>
          )}
        </Section>
        ...
        <Section id="tickets" title="نظام التذاكر" icon="🎫" activeSection={activeSection} setActiveSection={setActiveSection}>
          <div onClick={() => setRequireTicket((v: boolean) => !v)}
            className={`flex items-center justify-between p-4 rounded-xl border cursor-pointer transition-all select-none ${requireTicket ? 'bg-purple-500/10 border-purple-500/30 hover:bg-purple-500/15' : 'bg-gray-900/40 border-gray-700/30 hover:border-gray-600/50'}`}>
            <div className="flex items-center gap-3">
