...
export default function ReservationsPage() {
  ...
  // ══ Active/Planned Activities ══
  const activeActivities = useMemo(() => {
    return activities.filter(a => a.status === 'planned' || a.status === 'active');
  }, [activities]);
    ...
    <div className="space-y-4 max-w-2xl mx-auto" dir="rtl">
      ...
      <div className="flex gap-2">
        <select
          ...
          onChange={e => { setFilterActivity(e.target.value); if (!formActivity) setFormActivity(e.target.value !== 'all' ? e.target.value : ''); }}
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 text-sm">لا توجد حجوزات{filterStatus !== 'all' || filterActivity !== 'all' ? ' مطابقة للفلتر' : ' بعد'}</p>
          </div>
        ) : (
          filtered.map(r => {
            ...
            return (
              <motion.div
                ...
                className={`bg-gray-800/30 border rounded-xl p-3.5 transition-all ${cfg.border}`}
              >
                {/* الصف الأول: الاسم + Badge */}
