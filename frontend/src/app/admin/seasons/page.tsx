...
export default function SeasonsPage() {
  ...
  const activeOnline = seasons.find(s => s.type === 'ONLINE' && s.status === 'ACTIVE');
  ...
  async function startOnline() {
    setBusy(true); setError('');
    try {
      await apiFetch('/api/seasons/online/start', { method: 'POST', body: JSON.stringify({ name: onlineName.trim() }) });
      setShowOnline(false); setOnlineName(''); await load();
    } catch (e: any) { setError(e.message); } finally { setBusy(false); }
  }
    ...
    <div dir="rtl" className="min-h-screen bg-[#0a0a0a] text-white p-4 md:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-b from-[#0e1a2e] to-[#0a0f16] border border-sky-500/30">
            <p className="text-[10px] font-mono tracking-widest text-[#808080] uppercase mb-1">🌐 موسم الأونلاين النشط (منفصل — لا يمسّ الرانك الوجاهيّ)</p>
            {activeOnline ? (
              <>
                <p className="text-xl font-black text-sky-300">{activeOnline.name}</p>
                <p className="text-xs text-[#888] mt-1">موسم #{activeOnline.seasonNumber} · {activeOnline.matchCount} مباراة · منذ {new Date(activeOnline.startedAt).toLocaleDateString('ar')}</p>
              </>
            ) : <p className="text-[#888] text-sm">لا يوجد موسم أونلاين نشط</p>}
            <button onClick={() => { setShowOnline(true); setError(''); }} disabled={busy}
              className="mt-4 w-full py-2.5 rounded-xl border border-sky-500/60 text-sky-300 font-bold text-sm hover:bg-sky-500/15 disabled:opacity-50">
              🌐 بدء موسم أونلاين جديد
            </button>
          </div>
        </div>
      ...
      <AnimatePresence>
        ...
        {showOnline && (
          <Modal onClose={() => setShowOnline(false)}>
            <h2 className="text-lg font-black mb-2 text-sky-300">🌐 بدء موسم أونلاين جديد</h2>
            <p className="text-xs text-[#888] mb-3 leading-relaxed">مواسم الأونلاين مستقلّة تماماً عن الرانك الوجاهيّ ومواسم البطولات — كل مباريات اللعب عن بُعد تُحتسب لموسم الأونلاين النشط فقط، ولا تمسّ نقاط اللاعبين الوجاهيّة إطلاقاً. بدء موسم أونلاين جديد يُنهي السابق (تبقى إحصاءاته محفوظة) ولا يصفّر أيّ رانك وجاهيّ.</p>
            <input value={onlineName} onChange={e => setOnlineName(e.target.value)} placeholder="اسم موسم الأونلاين (مثل: أونلاين — صيف)"
