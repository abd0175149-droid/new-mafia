...
// ══════════════════════════════════════════════════════
// 🪑 قسم إعدادات الجلوس الذكي
// ══════════════════════════════════════════════════════
const CONSTRAINT_LABELS: Record<string, { label: string; icon: string; desc: string }> = {
  NEW_PLAYER_SEPARATION: {
    label: 'فصل اللاعبين الجدد',
    icon: '👶',
    desc: 'لاعب جديد (< 3 فعاليات) لا يُحاط بلاعبَين جديدَين',
  },
  ...
  HIGH_RANK_SEPARATION: {
    ...
    icon: '⚔️',
    desc: 'لاعبان بـ RR عالٍ لا يجلسان بجانب بعض',
  },
  GENDER_SEPARATION: {
    label: 'فصل الجنسين',
    icon: '🚹',
...
function SeatingConstraintsSection({ gameState, emit, setError }: {
  gameState: any;
  ...
  const [reshuffleResult, setReshuffleResult] = useState<string | null>(null);

  const activityId = gameState.activityId;

  // جلب الإعدادات عند فتح القسم
  useEffect(() => {
    if (!expanded || !activityId) return;
    fetchConstraints();
  }, [expanded, activityId]);
  ...
  const fetchConstraints = async () => {
    setLoading(true);
    try {
      const token = localStorage.getItem('token') || localStorage.getItem('leader_token') || '';
      const res = await fetch(`${API_URL}/api/seating/constraints?activityId=${activityId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      setEngineEnabled(data.engineEnabled || false);
  ...
  const saveConstraints = async (newEnabled: boolean, newConstraints: any[]) => {
    try {
      const token = localStorage.getItem('token') || localStorage.getItem('leader_token') || '';
      await fetch(`${API_URL}/api/seating/constraints`, {
        method: 'PUT',
  ...
  const handleReshuffle = async () => {
    ...
    try {
      const token = localStorage.getItem('token') || localStorage.getItem('leader_token') || '';
      const res = await fetch(`${API_URL}/api/seating/reshuffle`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({
      ...
      const data = await res.json();
      if (data.success) {
        setReshuffleResult(`✅ تم إعادة الترتيب — ${data.violations || 0} مخالفة`);
      } else {
        setReshuffleResult(`❌ ${data.error || 'فشل إعادة الترتيب'}`);
      }
    } catch (err: any) {
      setReshuffleResult(`❌ ${err.message}`);
    } finally {
      setReshuffling(false);
      // مسح الرسالة بعد 5 ثوانٍ
      setTimeout(() => setReshuffleResult(null), 5000);
    }
  };

  // لا نعرض شيء إذا لم يكن مرتبط بنشاط
  ...
  return (
    <div className="flex flex-col items-center gap-2 w-full max-w-md">
      <button
        ...
        className="flex items-center gap-2 text-[#808080] text-[10px] font-mono tracking-widest uppercase hover:text-[#C5A059] transition-colors"
      >
        🪑 SEATING ENGINE
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            <div className="bg-[#050505] rounded-xl border border-[#2a2a2a] p-4 space-y-4">
              {loading ? (
                <div className="flex justify-center py-4">
                  <div className="animate-spin h-5 w-5 border-2 border-[#C5A059] border-t-transparent rounded-full" />
                </div>
              ) : (
                <>
                  {/* Engine Toggle */}
                  <div className="flex items-center justify-between">
                    <span className="text-white text-xs font-mono">المحرك الذكي</span>
                    <button
                      onClick={toggleEngine}
                      className={`px-4 py-1.5 rounded-lg text-[10px] font-mono uppercase tracking-widest transition-all border ${
                        engineEnabled
                          ? 'bg-emerald-900/30 border-emerald-500/40 text-emerald-400'
                          : 'bg-[#111] border-[#2a2a2a] text-[#666]'
                      }`}
                    >
                      {engineEnabled ? '✓ ACTIVE' : 'OFF'}
                    </button>
                  </div>
                  ...
                  {engineEnabled && (
                    <div className="space-y-2">
                      {constraints.map(c => {
                        const meta = CONSTRAINT_LABELS[c.type] || { label: c.type, icon: '⚙️', desc: '' };
                        return (
                          <div
                            key={c.type}
                            className={`flex items-center justify-between bg-[#0a0a0a] border rounded-lg px-3 py-2.5 transition-all ${
                              c.enabled ? 'border-[#C5A059]/30' : 'border-[#1a1a1a]'
                            }`}
                          >
                            <div className="flex items-center gap-2 flex-1 min-w-0">
                              <span className="text-sm shrink-0">{meta.icon}</span>
                              <div className="min-w-0">
                                <p className={`text-[11px] font-mono truncate ${c.enabled ? 'text-white' : 'text-[#555]'}`}>
                                  {meta.label}
                                </p>
                                <p className="text-[8px] text-[#444] truncate">{meta.desc}</p>
                              </div>
                            </div>
                            <button
                              onClick={() => toggleConstraint(c.type)}
                              className={`w-10 h-5 rounded-full relative transition-all shrink-0 ${
                                c.enabled ? 'bg-[#C5A059]/30' : 'bg-[#1a1a1a]'
                              }`}
                            >
                              <div
                  ...
                  {/* Reshuffle Button */}
                  {engineEnabled && gameState.players.filter((p: any) => !p.seatHeld).length >= 2 && (
                    <button
                      onClick={handleReshuffle}
                      disabled={reshuffling}
                      className="w-full py-2.5 bg-[#111] border border-[#C5A059]/30 text-[#C5A059] rounded-lg text-[10px] font-mono uppercase tracking-widest hover:bg-[#C5A059]/10 transition-colors disabled:opacity-50"
                    >
                      {reshuffling ? '⏳ جارٍ إعادة الترتيب...' : '🔄 RESHUFFLE ALL SEATS'}
                    </button>
                  )}
                  ...
                  {reshuffleResult && (
                    <p className={`text-[10px] font-mono text-center ${
                      reshuffleResult.startsWith('✅') ? 'text-emerald-400' : 'text-red-400'
                    }`}>
                      {reshuffleResult}
                    </p>
                  )}
