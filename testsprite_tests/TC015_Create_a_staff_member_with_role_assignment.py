...
async def run_test():
    ...
    try:
        ...
        # -> Fill the admin username field with 'admin' (then fill password and submit the form).
        frame = context.pages[-1]
        # Input text
        ...
        # -> Click the 'الموظفون' (Staff) navigation item to open staff management and view the staff list.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/aside/nav/a[6]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the '+ إضافة موظف جديد' button to open the add-staff form/modal.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/main/div/div/div/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Fill the staff full name field (index 862) with 'Staff Test User', then fill username and password, save, and verify the new staff row appears in the staff list.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div/main/div/div/div[3]/div/div[2]/div/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('Staff Test User')
        
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div/main/div/div/div[3]/div/div[2]/div[3]/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('staff.test.user@example.com')
        ...
        # -> Click the 'حفظ التغييرات' (Save changes) button to submit the new staff form, then verify the new staff entry appears in the staff list.
        frame = context.pages[-1]
        # Click element
