...
### ن8 — الجذر المُشخَّص لعطل تنبيه الليدر (تكملةٌ لِـن7)

**البلاغ**: لا يظهر تنبيه مغادرة اللاعب في واجهة الليدر، مع أن القيمة تظهر
في داشبورد مكافحة الغش. وهذا التناقض بعينه هو الدليل: الإشارة **تصل وتُخزَّن**
ولا تُرسم.

تُتبِّعت الحلقات كلُّها فكانت سليمة، والعطل في الأخيرة وحدها:

| الحلقة | الحالة |
|---|---|
| العميل يبثّ `cheat:app-departure` | ✅ |
| الخادم يزن ويخزّن في `cheat_signals` | ✅ (لذلك يظهر في الداشبورد) |
| الخادم يبثّ `leader:cheat-signal` لسوكت الليدر | ✅ `lobby.socket.ts` |
| صفحة الليدر تستمع | ✅ |
| **الرسم** | ❌ |

🔴 لوحة `cheatFeed` كانت مرسومةً في **كتلة العودة الأخيرة وحدها** — شاشة
إنشاء اللعبة — وهي التي **يستحيل** أن تصل فيها إشارة، إذ يشترط
`recordCheatSignal` لعبةً جاريةً غير `GAME_OVER`. وأثناء اللعب يعرض الليدر
شاشة الجلسة التي لم تكن تحوي اللوحة.

🔴 **ولماذا التجميع ضرورةٌ لا تحسين**: الإشارة الواحدة لا تُدين والنمط يُدين
— لاعبٌ غادر مرّةً لثانيتين ≠ لاعبٌ غادر خمساً وشاشة السرّ مفتوحة. والسطر
العابر يختفي بعد ١٥ث، فيضيع النمط حتى لو رُسم. لذلك بقي السطر العابر **فوق**
اللوحة الدائمة: الأوّل للانتباه اللحظيّ والثانية للحكم على النمط.

### ن9 — عقبة بيئيّة على الـMac: `npm` وسلسلة الشهادات

`npm` يسقط بـ`UNABLE_TO_GET_ISSUER_CERT_LOCALLY` — اعتراضٌ في الشبكة يشبه ما
أسقط حزمة RealtimeKit (ن1 في الملفّ 97).

🔴 **لا يُعالَج بتعطيل `strict-ssl`**: ذلك يجعل كلّ حزمةٍ تُنزَّل غير موثّقة
المصدر — بابُ تسميم سلسلة التوريد. العلاج الصحيح تصدير جذور شهادات النظام
(١٦١ جذراً) وتمريرها لـNode مع إبقاء التحقّق كاملاً:
...
export NODE_EXTRA_CA_CERTS=$PWD/roots.pem
```

بعدها ثُبِّت Node `v26.7.0` ومرّ `tsc --noEmit` بصفر أخطاء و`npm run build`
بنجاح وبُنيت صفحة `/leader` — تأكيدٌ مستقلٌّ لِما أثبتته ن7 على ويندوز.
