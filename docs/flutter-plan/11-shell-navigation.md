...
- الويب يضيف `backdrop-filter: blur(20px)` — في Flutter الخلفية شبه معتمة أصلاً؛ **يُستغنى عن الـ blur** ويُستخدم التدرج فقط (انظر §13).
...
- **بلا BackdropFilter**: بطاقات `#0c0c0c/90 + blur` وخلفية الشريط `blur(20px)` تُستبدل بألوان شبه معتمة مسبقة الدمج (`#E60C0C0C` للبطاقات، التدرج المعتم للشريط) — الـ blur الحي مكلف على تابلتات الأندرويد الضعيفة المستخدمة في النادي، والفرق البصري على خلفية `#050505` شبه معدوم.
