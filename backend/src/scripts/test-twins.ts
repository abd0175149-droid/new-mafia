...
// ══════════════════════════════════════════════════════
section('2) التعارف — تعارف أحادي الاتجاه (الأكبر فقط يرى أخاه؛ الأصغر أعمى تماماً)');
{
  const s = makeState();

  // الأخ الأكبر (مافيا) — يرى أخاه الأصغر
  ...
  // الأخ الأصغر (مواطن) — 🔒 لا يُكشف له أخوه الأكبر إطلاقاً (وإلّا عرف أنه مافيا
  // فقد يتعمّد إخراجه ليتحوّل بدلاً منه — تعارض مصالح). يلعب كمواطن أعمى.
  const younger = recognitionFor(s, 6);
  check('🔒 الأصغر: لا يحصل sibling إطلاقاً (تعارف أحادي)', younger.sibling === undefined);
  check('🔒 الأصغر: لا يعرف أنّ أخاه مافيا (لا بيانات أخ)', younger.sibling === undefined);
...
section('13) التعارف بعد التحوّل — الأصغر المتحوّل يرى الأكبر (الميت) كأخ');
{
  ...
  // 🔒 التعارف أحادي الاتجاه يبقى سارياً حتى بعد التحوّل: الأصغر لا يُكشف له أخوه إطلاقاً
  // (يكتشف انضمامه للمافيا عبر إشعار التحوّل، لا عبر بطاقة الأخ).
  check('13: الأصغر (المتحوّل) لا يحصل sibling — تعارف أحادي', getSiblingInfoFor(s, 6) === null);
}
