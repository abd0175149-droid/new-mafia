...
export const soundEffects = pgTable('sound_effects', {
  ...
  // ⏱ مدّةُ التشغيل القصوى لكلّ حدثٍ يخدمه هذا الملفّ: { "ambient_night": 8000 }
  //    غيابُ المفتاح = المقطعُ كاملاً. لكلّ حدثٍ مدّتُه لأنّ الملفّ الواحد قد يخدم
  //    حدثاً يحتاج ثانيتين وآخرَ يحتاج دقيقة — فمدّةُ الملفّ وحدَه لا تكفي.
...
export const waBotUsage = pgTable('wa_bot_usage', {
  ...
  // 📈 قياس الجودة (منذ 2026-09-19): زمن الردّ، الأدوات المستدعاة، وأعلام (unknown/handoff/leak/fail/audio)
  replyMs: integer('reply_ms'),
  tools: jsonb('tools').default([]),
  flags: jsonb('flags').default({}),                  // من promptTokens — خُدمت من كاش المزوّد وتُفوتَر بخصم
  createdAt: timestamp('created_at').defaultNow().notNull(),
});
