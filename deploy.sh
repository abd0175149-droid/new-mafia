...
# ── 1ب. السكربتُ يُحدّث نفسه — فيجب أن يُعيد قراءتها ──
#
# 🔴 علّةٌ صامتة كلّفتنا ترحيلَين: bash يقرأ السكربت على دفعات، فحين يسحب
#    `git pull` نسخةً جديدة من deploy.sh تكون كتلةُ الترحيل (أسفل) قد قُرئت
#    من النسخة **القديمة**. فيُنفَّذ ترحيلُ الأمس، ويُعلن «✅ الترحيل تمّ»
#    بصدق — عن ترحيلٍ ليس فيه ما أُضيف. لا خطأ، ولا تحذير، ولا أثر.
#
#    ظهر مرّتين: جدولُ player_fixes لم يُنشأ، ثمّ قيدُه الفريد. وفي كلتيهما
#    كشفه سؤالٌ عابر لا السكربت، وطُبِّق الترحيل يدويّاً.
#
#    فإن تغيّر السكربت في السحب، تُستبدل العمليّة بنسخته الجديدة مرّةً واحدة.
#    الحارسُ يمنع الدوران، والنسخةُ الاحتياطيّة تتكرّر — ثمنٌ زهيد.
if [ "$SELF_BEFORE" != "$(sha256sum "$0" | cut -d' ' -f1)" ] && [ -z "${DEPLOY_REEXEC:-}" ]; then
  say ""
  say "   ♻️  سكربتُ النشر نفسه تغيّر — يُعاد تنفيذه بنسخته الجديدة"
